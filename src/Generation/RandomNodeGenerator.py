...
class RandomNodeGenerator1d:
    def __init__(self, numNodes, bounds=[]):
        ...
        if len(self.bounds) == 0:
            self.bounds = [-1, 1]  # default bounds if none are provided
        import random
        for i in range(numNodes):
            self.positions.append(random.uniform(self.bounds[0], self.bounds[1]))
...
class RandomNodeGenerator2d:
    def __init__(self, numNodes, bounds=[]):
        ...
        self.positions = []
        if len(self.bounds) == 0:
            self.bounds = [[-1, -1], [1, 1]]  # default bounds if none are provided
        import random
        for i in range(numNodes):
...
class RandomNodeGenerator3d:
    def __init__(self, numNodes, bounds=[]):
        ...
        self.positions = []
        if len(self.bounds) == 0:
            self.bounds = [[-1,-1,-1], [1, 1, 1]]  # default bounds if none are provided
        import random
        for i in range(numNodes):
            x = random.uniform(self.bounds[0][0], self.bounds[1][0])
            y = random.uniform(self.bounds[0][1], self.bounds[1][1])
