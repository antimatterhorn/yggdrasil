...
if __name__ == "__main__":
    commandLine = CommandLineArguments(numNodes = 300,
                                       angleTol = 25.0)

    positions = ConstantDThetaDisk2d(numNodes).positions
    ...
    for edge, (t1, t2) in interior_edges:
        ...
        pts = points[quad_verts]

        if not is_convex(pts):
            continue

        angles = polygon_angles(pts)
        if np.all(np.abs(angles - 90.0) <= angleTol):
    ...
    # ---------------------------------------------------------------
    # 5. Plot: quads in one color, leftover triangles in another
    # ---------------------------------------------------------------
    fig, ax = plt.subplots(figsize=(7, 7))

    for t in leftover_triangles:
        poly = MplPolygon(points[t], closed=True, fill=True,
