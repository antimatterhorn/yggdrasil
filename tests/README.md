...
| [kelvin-helmholtz.py](kelvin-helmholtz.py) | Kelvin–Helmholtz shear-layer instability: three density/velocity bands with a sinusoidal velocity perturbation, evolved with `GridHydroKT2d` and periodic boundaries, dumped to Silo; also drops a `RestartWriter2d` checkpoint every `restartCycle` steps and picks it up via `RestartReader2d` on startup if one is present, so re-running the script resumes instead of restarting from the initial conditions |
