...
| [noh.py](Physics/noh.py) | Planar Noh problem: two cold streams collide head-on, driving a pair of stagnation shocks; run to a fixed `tstop` with all three grid solvers (KT, HLLC, HLLE), each checked against the exact solution (`AnalyticSolutions.NohSolution`) and all three overplotted with it |
| [nohRZ.py](Physics/nohRZ.py) | Cylindrical `(r,z)` Noh problem: cold gas converges radially onto the axis; all three grid solvers checked/plotted against the same exact solution, including the convergent pre-shock `1 + v0 t/r` profile unique to non-planar geometry |
...
| [sedov.py](Physics/sedov.py) | Sedov–Taylor point-blast test: a Gaussian energy deposition in a uniform-density 2D Cartesian grid (cylindrical symmetry, reflecting boundaries), run to a fixed `tstop` with all three grid solvers (KT, HLLC, HLLE), each checked against the closed-form self-similar solution (`AnalyticSolutions.SedovSolution`) plus total-energy conservation, and all three overplotted with it |
| [sedovRZ.py](Physics/sedovRZ.py) | Cylindrical `(r,z)` Sedov point blast at the axis/equatorial-wall corner, representing a true spherical explosion; all three grid solvers checked/plotted against the same self-similar solution, alongside finite/expansion/mass-conservation invariants |
| [sod.py](Physics/sod.py) | Classic Sod shock-tube test: a density/energy step evolved with all three grid solvers (`GridHydroKT2d`, `GridHydroHLLC2d`, `GridHydroHLLE2d`) under reflecting boundaries, run to a fixed `tstop`, each checked against the exact Riemann solution (`AnalyticSolutions.SodSolution`) and all three overplotted with it |
