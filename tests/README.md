...
| [sedov.py](Physics/sedov.py) | Sedov–Taylor point-blast test: a Gaussian energy deposition in a uniform-density grid, evolved with `GridHydroKT2d` under reflecting boundaries and plotted along the mid-line |
| [sod.py](Physics/sod.py) | Classic Sod shock-tube test: a density/energy step evolved with `GridHydroKT2d` under reflecting boundaries, animated and plotted along the mid-line |
